...
<!--文件体开始-->
		...
		<tr>
       	...
       	<c:forEach items="${shoppingcart.lines}" var="map">
        	<tr>
				...
				<td class=tablebody1 valign=middle align=center width=""><input type="text" name="num" value="${map.value.num }" size="4" onblur="javascript:if(this.value<1){alert('对不起，产品数量不能小于 1 ');this.focus();}else{number.value=this.value;}"/></td>
				<td class=tablebody2 valign=middle align=center width="">${map.value.book.price*map.value.num}</td>
				<td class=tablebody1 valign=middle align=center width="">
				<input type="button" value="取消" onclick="javascript:window.location='removeProduct.action?productid=${map.key}';"> <input type="submit" value="保存修改"></td>
				</form>
			</tr>
       	</c:forEach>
		...
		<tr>
			...
			<td class=tablebody1 valign=middle align=center width=""><font color="#ff0000"><b>${shoppingcart.cost}</b></font></td>
