...
<!--文件体开始-->
		...
		<tr>
			...
			<td width="60%" class="tablebody1">${user.name }</td>
		</tr>
		<tr>
			<td class="tablebody2" align="right"><b>联系地址</b>：</td>
			<td class="tablebody1">${user.address }</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td class="tablebody1">${user.telephone }</td>
		</tr>
		<tr>
...
<!--文件尾开始-->
        ...
        <c:forEach items="${shoppingcart.lines }" var="map">        
			<tr>
				...
				<td class=tablebody1 valign=middle width="">&nbsp;&nbsp;<a href="user/productDetail?id=${map.key }" target="_blank">产品ID：${map.value.book.id }</a></td>
				...
				<td class=tablebody2 valign=middle align=left width="">小计：${map.value.num*map.value.book.price }</td>
			</tr>
        </c:forEach>
